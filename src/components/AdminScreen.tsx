...
export default function AdminScreen({ onBackToGame }: AdminScreenProps) {
  ...
  const [activeTab, setActiveTab] = useState<'history' | 'users' | 'settings'>('history');
    ...
    <div className="w-full max-w-6xl mx-auto space-y-8" id="admin-panel-container">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="metrics-panel">
        ...
        <div className="bg-white border-4 border-black p-6 neo-shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="font-mono text-[10px] font-black uppercase text-zinc-400">TOTAL LOGINS</p>
      ...
      {/* Main Tab Navigation */}
      <div className="flex flex-col sm:flex-row border-b-4 border-black gap-0 bg-transparent">
        <button
          type="button"
        ...
        >
          ...
          01 // CALCULATION CORRESPONDENCE LOGS
        </button>

        <button
          type="button"
          onClick={() => setActiveTab('users')}
          className={`py-4 px-6 font-display text-lg uppercase tracking-wider transition-colors cursor-pointer border-t-4 border-r-4 border-black sm:border-l-0 border-l-4 -mb-[4px] relative z-10 flex items-center justify-center gap-2 ${
          ...
          <div className="space-y-6 animate-fade-in animate-duration-100" id="registered-users-panel">
            <div className="flex justify-between items-center bg-black text-white p-4">
              <h3 className="font-mono text-xs uppercase tracking-widest font-black flex items-center gap-2">
                ...
                // PRIVILEGED ACCOUNTS REGISTRY ({usersList.length} LOGINS RECORDED)
              </h3>
            </div>
